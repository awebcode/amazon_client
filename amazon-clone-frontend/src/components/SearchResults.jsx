...
//       if (i < filledCount) {
//         icons.push(
//           <StarIcon key={i} style={{ fontSize: "18px" }} className="text-yellow-300" />
//         );
...
//                 </li>
//               ))}
//           {/* Show More button to display all categories */}
//           {subcategories.length > numDisplayedCategories && (
//             <li
//               className="category-link list-disc text-[8px] md:text-[16px] cursor-pointer text-blue-500"
...
const SearchResults = () => {
  ...
  const getSearchResults = () => {
    ...
    if (myproducts) {
      ...
      if (category) {
        categoryResults = myproducts.filter((product) => product.category === category);
      }
    ...
    for (let i = 0; i < 5; i++) {
      if (i < filledCount) {
        ...
        icons.push(
          <StarIcon
            key={i}
            style={{ opacity: 0.5, fontSize: isMobile ? "9px" : "11px" }}
            className="text-gray-400"
          />
        );
      }
    ...
    <>
      ...
      <div className="p-3 md:p-10 container flex justify-between items-start">
        <ul className="categoryBox">
          ...
          <Typography className="m-[20px]">
            <Link to="/products" className="text-[10px] md:text-[12px]">
              <h1 className="text-[10px] md:text-[12px] font-bold my-4">
                {category} Categories
              </h1>
            </Link>
          </Typography>
          {subcategories &&
            subcategories
              ...
              .map((subcategory, i) => (
                <li
                  className="category-link list-disc text-[8px] md:text-[12px]"
                  key={i}
                  onClick={() => {
          ...
          {subcategories.length > numDisplayedCategories && (
            <li
              className="category-link list-disc text-[8px] md:text-[12px] cursor-pointer text-blue-500"
              onClick={toggleCategoriesDisplay}
            >
              {showAllCategories ? "Show Less" : "Show More"}
            </li>
          ...
          <div>
            ...
            <h1 className="text-[12px] md:text-[16px] font-bold my-4">Filter by price</h1>
            <ul className="space-y-2">
              {priceOptions.map((option) => (
                <li
                  key={option.value}
                  className={`cursor-pointer text-[8px] md:text-[12px] ${
                    selectedPrice === option.value ? "font-bold" : ""
                  }`}
                  onClick={() => setSelectedPrice(option.value)}
                >
